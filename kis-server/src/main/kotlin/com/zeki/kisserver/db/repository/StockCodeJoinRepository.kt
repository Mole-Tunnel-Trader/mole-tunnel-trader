...
class StockCodeJoinRepository(
    ...
    fun bulkInsert(stockCodeSaveList: Collection<com.zeki.kisserver.db.entity.StockCode>) {
        stockCodeSaveList.chunked(batchSize).forEach {
            this.bulkInsertUsingBatch(it)
        }
    }

    private fun bulkInsertUsingBatch(stockCodeSaveList: Collection<com.zeki.kisserver.db.entity.StockCode>) {
        var sql = buildString {
    ...
    fun bulkUpdate(stockCodeUpdateList: Collection<com.zeki.kisserver.db.entity.StockCode>) {
        stockCodeUpdateList.chunked(batchSize).forEach {
            this.bulkUpdateUsingBatch(it)
        }
    }

    private fun bulkUpdateUsingBatch(stockCodeUpdateList: Collection<com.zeki.kisserver.db.entity.StockCode>) {
